...
public class ExpenseService {
	public User settleBalance(String user_id, List<Expense> expenseList){
		...
		for (Expense expense:expenseList) {
			...
			if (lendToMap.containsKey(expense.getUser_id())){
				...
				settleOwersPreviousExpenseOnCurrentUser( user_id, expense);
	...
	void settleOwersPreviousExpenseOnCurrentUser(String user_id,Expense expense){
		Expense ownerPreviousExpense = UserRepository.getStringUserMap().get(expense.getUser_id()).getLendToMap().get(user_id);

		double diffAmount = ownerPreviousExpense.getAmount()-expense.getAmount();
		if(diffAmount>0){
			expense.setAmount(diffAmount);
			ownerPreviousExpense.setAmount(-diffAmount);
		}else{
			expense.setAmount(-diffAmount);
			ownerPreviousExpense.setAmount(diffAmount);
		}
		UserRepository.getStringUserMap().get(user_id).getLendToMap().put(expense.getUser_id(),expense);
		UserRepository.getStringUserMap().get(expense.getUser_id()).getLendToMap().put(user_id,ownerPreviousExpense);
	...
	List<Expense> getExpenseList(String lender,List<String> borrowers, List<Double> spendAmount){
		...
		int i =0;
		for(String borrwer: borrowers){
			if(lender.equals(borrwer))
				continue;
			expenseList.add(new Expense(borrwer,spendAmount.get(i++)));
		}
		return expenseList;
	}
}
